...
public class Website {
    ...
    */
    ...
    public static boolean registrations(String login, String password, String confirmPassword) {
        try {
            if (login.length() >= 20 || login.matches(".*\\s.*|\\D*")) {
                throw new WrongLoginException("Логин не соответствует требованиям");
            }

            if (password.length() >= 20 || password.matches(".*\\s.*|\\D*")) {
